...
export default function NBList() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [nbRes, memberRes, gpRes, gcRes, bcRes, customerRes] = await Promise.all([
        apiFetch(
          getQueryUrl(API_CONFIG.ENDPOINTS.NATIONAL_BRAND, { fields: ["*"], limit: 10000000 }),
        ...
        apiFetch(
          getQueryUrl(API_CONFIG.ENDPOINTS.GROUP_PARENT, { fields: ["*"], limit: 10000000 }),
          { method: "GET", cache: "no-store" },
          token
        ),
        apiFetch(
          getQueryUrl(API_CONFIG.ENDPOINTS.GROUP_CUSTOMER, { fields: ["*"], limit: 10000000 }),
          { method: "GET", cache: "no-store" },
          token
        ),
        apiFetch(
      ...
      if (!nbRes.ok) throw new Error(`Failed to fetch national brands (${nbRes.status})`);
      if (!memberRes.ok) throw new Error(`Failed to fetch member_of (${memberRes.status})`);
      ...
      const [nbJson, memberJson, gpJson, gcJson, bcJson, customerJson] = await Promise.all([
        nbRes.json(),
        memberRes.json(),
        gpRes.json(),
        gcRes.json(),
      ...
      const nbRows: NationalBrandApiResponse[] = Array.isArray(nbJson?.data) ? nbJson.data : [];
      const memberRows: MemberOfApiResponse[] = Array.isArray(memberJson?.data) ? memberJson.data : [];
      const gpRows: GroupParentApiResponse[] = Array.isArray(gpJson?.data) ? gpJson.data : [];
      const gcRows: GroupCustomerApiResponse[] = Array.isArray(gcJson?.data) ? gcJson.data : [];
      const bcRows: BranchCustomerApiResponse[] = Array.isArray(bcJson?.data) ? bcJson.data : [];
      const customerRows: CustomerRegisterOwnerRow[] = Array.isArray(customerJson?.data)
        ? customerJson.data
        : [];
      ...
      memberRows.forEach((row) => {
        ...
        if (!nbId) return;
        const rawName =
          typeof row.user === "object" ? row.user?.full_name : row.user ? `User ${row.user}` : undefined;
      ...
      const mapped: NationalBrandItem[] = nbRows.map((row) => {
        const gpCandidates = (gpByNb.get(row.id) || []).filter((gp) => asActive(gp.disabled));
        const gpIds = gpCandidates.map((gp) => gp.id);

        const gcCandidates = gpIds.flatMap((gpId) => gcByGp.get(gpId) || []).filter((gc) => asActive(gc.disabled));
        const gcIds = gcCandidates.map((gc) => gc.id);

        const bcCandidates = gcIds.flatMap((gcId) => bcByGc.get(gcId) || []).filter((bc) => asActive(bc.disabled));
  ...
  return (
    <div>
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">National Brand (NB)</h1>
          <p className="text-sm md:text-base text-gray-600">Kelola data National Brand</p>
        </div>
      </div>
                    ...
                    {[
                      ...
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-sm font-medium hover:bg-gray-50 transition-colors ${
                          sortField === option.value ? "text-indigo-600 bg-indigo-50" : "text-gray-700"
                        }`}
                      >
                        {option.label}
                      </button>
      ...
      {filteredAndSorted.length === 0 && (
        <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-gray-100">
          ...
          <h3 className="text-lg font-semibold text-gray-800 mb-2">Tidak ada NB ditemukan</h3>
          <p className="text-sm text-gray-500">
            {searchQuery ? "Coba ubah kata kunci pencarian" : "Belum ada data National Brand"}
          </p>
        </div>
      )}
              ...
              >
                ...
                <div className="p-5 text-sm space-y-3">
                  <div className="flex items-start gap-2 text-gray-700">
                    ...
                    <div>
                      <p className="text-xs text-gray-500">Owner / Pengguna NB</p>
                      <p className="font-medium text-gray-900 line-clamp-2">
                        {item.owners.length > 0 ? item.owners.join(", ") : "Belum ada owner di member_of"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div className="rounded-lg border border-purple-100 bg-purple-50 px-2 py-2 text-center">
                      ...
                      <div className="mt-1 text-purple-900 font-bold">{item.active_gp_count}</div>
                    </div>
                    <div className="rounded-lg border border-blue-100 bg-blue-50 px-2 py-2 text-center">
                      ...
                      <div className="mt-1 text-blue-900 font-bold">{item.active_gc_count}</div>
                    </div>
                    <div className="rounded-lg border border-orange-100 bg-orange-50 px-2 py-2 text-center">
                      ...
                      <div className="mt-1 text-orange-900 font-bold">{item.active_bc_count}</div>
                    </div>
                  </div>

                  <div className="text-xs text-gray-600 space-y-1">
                    <p>
                      <span className="font-semibold text-gray-700">PT/GP aktif:</span>{" "}
                      {item.sample_gp_names.length > 0 ? item.sample_gp_names.join(", ") : "-"}
                    </p>
                    <p>
                      <span className="font-semibold text-gray-700">Child/GC aktif:</span>{" "}
                      {item.sample_gc_names.length > 0 ? item.sample_gc_names.join(", ") : "-"}
                    </p>
                  </div>
